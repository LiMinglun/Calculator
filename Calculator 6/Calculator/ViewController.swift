...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
  ...
  //  let calcN = 8000
    ...
    func buttonTapped(btn: UIButton){
      //  print(expressionA)
        ...
        if btn.titleLabel!.text! == "- " && isPurnFloat(string: expressionA[expressionA.count - 1]){
                return
            }
        }
    ...
    @IBAction func upTapped(_ sender: Any) {
        ...
        yMax.text = String(Double(yMax.text!)! + 1)
        yMin.text = String(Double(yMin.text!)! + 1)
        view.endEditing(true)
        drawCoordination()
    ...
    @IBAction func leftTapped(_ sender: Any) {
        if isPurnFloat(string: yMax.text!) && isPurnFloat(string: yMin.text!){
            xMax.text = String(Double(xMax.text!)! - 1)
            xMin.text = String(Double(xMin.text!)! - 1)
            view.endEditing(true)
            drawCoordination()
            if ViewController.isFuncGraphable{
                drawDiff()
    ...
    @IBAction func inTapped(_ sender: Any) {
        if isPurnFloat(string: yMax.text!) && isPurnFloat(string: yMin.text!){
            yMax.text = String(Int(Double(yMax.text!)! / 1.5 + 0.5))
            yMin.text = String(Int(Double(yMin.text!)! / 1.5 + 0.5))
            xMax.text = String(Int(Double(xMax.text!)! / 1.5 + 0.5))
            xMin.text = String(Int(Double(xMin.text!)! / 1.5 + 0.5))
            view.endEditing(true)
            drawCoordination()
            if ViewController.isFuncGraphable{
                drawDiff()
    ...
    func drawCoordination(){
        ...
        let screen_height = Double(UIScreen.main.bounds.height - UIApplication.shared.statusBarFrame.height-50)
        //find x ints
  ...
  /*      let tempArr = ViewController.yCP
        for i in 0...tempArr.count-1{
            ViewController.yCP[i][1] = tempArr[tempArr.count-1-i][1]
        }*/
        if yMinValue <= 0{
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        if let jpegData = UIImageJPEGRepresentation(gaussImage, 50) {
